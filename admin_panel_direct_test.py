#!/usr/bin/env python3
"""
MailGuard Admin Panel Direct Testing Suite
Tests admin panel functionality using direct database authentication
"""
...
class AdminPanelDirectTester:
    ...
    async def create_admin_token(self) -> bool:
        """Create admin token directly using database"""
        try:
            client = AsyncIOMotorClient(os.environ.get('MONGO_URL', 'mongodb://localhost:27017'))
            db = client[os.environ.get('DB_NAME', 'email_verifier_db')]
            ...
            print(f"✅ Created admin token for {admin['email']} (role: {admin['role']})")
            client.close()
            return True
    ...
    def authenticate_admin(self) -> bool:
        ...
        success = asyncio.run(self.create_admin_token())
        ...
        if success:
            ...
            if response and response.status_code == 200:
                ...
                self.log_result(category, "Direct token authentication", True, 
                              f"Authenticated as {user_data.get('email')} (role: {user_data.get('role')})")
                self.test_results[category]["status"] = "passed"
                return True
            else:
                self.log_result(category, "Token validation", False, 
                              f"Token validation failed: {response.status_code if response else 'No response'}")
        else:
            self.log_result(category, "Token creation", False, "Failed to create admin token")
    ...
    def run_all_tests(self):
        """Run all test suites"""
        print("🚀 Starting MailGuard Admin Panel Direct Tests")
