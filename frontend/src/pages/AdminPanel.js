  ...
  useEffect(() => {
    if (editing) {
      setFormData({
        ...
        yearly_price: editing.yearly_price || 0,
        yearly_original_price: editing.yearly_original_price || 0,
        yearly_discount_percentage: editing.yearly_discount_percentage || 0,
        price_usd: editing.price_usd || 0,
        currency: editing.currency || 'INR',
        billing_cycle: editing.billing_cycle || 'monthly',
        credits_limit: editing.credits_limit || 100,
        is_recurring: editing.is_recurring !== undefined ? editing.is_recurring : true,
        razorpay_plan_id_monthly: editing.razorpay_plan_id_monthly || '',
        razorpay_plan_id_yearly: editing.razorpay_plan_id_yearly || '',
          ...
          <div className="bg-white rounded-lg p-8 max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 mt-1">Optional: USD price for international customers</p>
              </div>
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label className="text-sm font-medium text-gray-700">Recurring Subscription</label>
              </div>

              {/* Razorpay Plan ID Linking Section */}
              <div className="border-t pt-4 mt-4">
                <h4 className="font-semibold text-gray-900 mb-3">Razorpay Subscription Plans</h4>
                <p className="text-xs text-gray-600 mb-3">Link Razorpay subscription plan IDs for automatic billing</p>
                
                <div className="space-y-3">
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Razorpay Plan ID (INR)</label>
                    <input
                      type="text"
                      value={formData.razorpay_plan_id_inr}
                      onChange={(e) => setFormData({...formData, razorpay_plan_id_inr: e.target.value})}
                    ...
                    />
                    <p className="text-xs text-gray-500 mt-1">For Indian customers (INR currency)</p>
                  </div>

                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Razorpay Plan ID (USD)</label>
                    <input
                      type="text"
                      value={formData.razorpay_plan_id_usd}
                      onChange={(e) => setFormData({...formData, razorpay_plan_id_usd: e.target.value})}
                      className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-600 focus:border-transparent font-mono text-sm"
                      placeholder="plan_xxxxxxxxxxxxx"
                    />
                    <p className="text-xs text-gray-500 mt-1">For international customers (USD currency)</p>
                  </div>
                </div>

                <div className="bg-blue-50 border border-blue-200 rounded p-3 mt-3">
                  <p className="text-xs text-blue-900">
                    💡 <strong>Tip:</strong> Create subscription plans in Razorpay Dashboard → Subscriptions → Plans, 
                    then paste the plan IDs here to enable recurring billing.
                  </p>
                </div>
              </div>
